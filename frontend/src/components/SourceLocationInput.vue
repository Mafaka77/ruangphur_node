<template>
  <div>
    ...
    <input
        v-model="formStore.second_step.source_locality"
    ...
    <label class="block text-sm font-bold text-black-700">
      Kilometer chhut dik a nih theih nan ruang chhuahna
    ...
    <div v-if="showMap" class="fixed inset-0 bg-white z-50 flex flex-col">
      <div class="flex justify-between items-center p-4 bg-gray-800 text-white">
        <h2 class="text-lg font-bold">Ruang chhuahna hmun zawng/thlang rawh</h2>
      ...
      <div class="p-4 flex items-center gap-2">
        <input
            ...
            type="text"
        />
        <button v-if="formStore.second_step.source_locality" class="px-4 py-2 bg-gray-300 rounded-md"
                @click="clearSearch">
          Clear
        </button>
      </div>
...
const initMap = () => {
  ...
  const initialLocation = {
    lat: formStore.second_step.source_lat || 23.726168909115387,
    lng: formStore.second_step.source_lng || 92.71900628064216,
  };
  ...
  autocomplete.addListener("place_changed", () => {
    ...
    formStore.second_step.source_lat = location.lat();
    formStore.second_step.source_lng = location.lng();
    formStore.second_step.source_locality = place.formatted_address || "";
...
const reverseGeocode = (location) => {
  geocoder.geocode({location}, (results, status) => {
    if (status === "OK" && results[0]) {
      ...
      formStore.second_step.source_lat = location.lat();
      formStore.second_step.source_lng = location.lng();
      formStore.second_step.source_locality = results[0].formatted_address;
    } else {
...
const clearSearch = () => {
  formStore.second_step.source_locality = "";
  if (marker) {
    marker.setMap(null);
    marker = null;
  }
